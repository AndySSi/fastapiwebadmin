...
class StatisticService:
    ...
    def index_count_statistic():
        ...
        # 运行用例报告统计 根据人
        report_statistic = (
            ApiTestReport.statistic_report()
            .group_by(ApiTestReport.execute_user_id)
            .order_by(text("run_num desc"))
            .limit(10)
            .all()
        )
    ...
    def get_case_exe_results():
        ...
        report_all = ApiTestReport.get_report_by_time(
            b_time.strftime("%Y-%m-%d 00:00:00"), today.strftime("%Y-%m-%d 23:59:59")
        ).all()
        ...
        for i in range(-11, 1):
            ...
            results["percent"].append(
                round(pass_count / total_run * 100, 2) if total_run != 0 else 0.00
            )
