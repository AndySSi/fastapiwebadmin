...
class User(Base):
    ...
    user_type = Column(
        Integer, nullable=False, comment="用户类型 10 管理人员, 20 测试人员", default=20
    )
    ...
    async def get_user_by_name(cls, username: str):
        stmt = select(*cls.get_table_columns()).where(
            cls.username == username, cls.enabled_flag == 1
        )
        return await cls.get_result(stmt, True)
    ...
    async def get_user_by_nickname(cls, nickname: str):
        stmt = select(*cls.get_table_columns()).where(
            cls.nickname == nickname, cls.enabled_flag == 1
        )
        return await cls.get_result(stmt, True)
...
class Department(Base):
    ...
    async def get_children(cls, parent_id: int):
        """获取子部门"""
        stmt = select(*cls.get_table_columns()).where(
            cls.parent_id == parent_id, cls.enabled_flag == 1
        )
...
class Menu(Base):
    ...
    async def get_menu_by_ids(cls, ids: typing.List[int]):
        ...
        stmt = (
            ...
            .where(cls.id.in_(ids), cls.enabled_flag == 1)
            .order_by(cls.sort)
        )
        return await cls.get_result(stmt)
...
class Roles(Base):
    ...
    menus = Column(
        Text, nullable=True, comment="菜单列表", index=False
    )  # 改为Text类型，支持更长的权限列表
...
class UserLoginRecord(Base):
    ...
    async def get_by_token(cls, token: str):
        ...
        stmt = (
            select(cls.get_table_columns())
            .where(cls.enabled_flag == 1, cls.token == token)
            .order_by(cls.id.desc())
        )
        return await cls.get_result(stmt, first=True)
