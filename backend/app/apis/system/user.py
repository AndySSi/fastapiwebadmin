...
async def debug_user_permissions():
    ...
    if user_info.user_type == 10:
        # 超级管理员
        debug_info["final_auth_list"] = [
            btn.get("roles") for btn in all_buttons if btn.get("roles")
        ]
    else:
        # 普通用户
        if user_info.roles:
            ...
            debug_info["user_roles"] = [
                {"id": r.get("id"), "name": r.get("name"), "menus": r.get("menus")} for r in roles
            ]
            ...
            if menu_ids:
                ...
                debug_info["final_auth_list"] = [
                    btn.get("roles") for btn in buttons if btn.get("roles")
                ]
