...
class PeriodicTaskChanged(ModelBase, ModelMixin):
    ...
    def update_changed(cls, mapper, connection, target):
        ...
        :param target: the mapped instance being persisted
        """
        s = connection.execute(select(cls).where(cls.id == 1).limit(1))
        if not s:
            s = connection.execute(insert(cls), last_update=dt.datetime.now())
        else:
            s = connection.execute(
                update(cls).where(cls.id == 1).values(last_update=dt.datetime.now())
            )
